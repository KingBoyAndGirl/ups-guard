...
| 🆕 | `ups.mfr.date` | `2025/11/21` | UPS生产日期 |
| 🔵 | `ups.model` | `Back-UPS BK650M2_CH` | UPS型号 |
| 🆕 | `ups.productid` | `0002` | USB产品ID |
| 🆕 | `ups.realpower.nominal` | `390` | 额定实际功率(W) |
| 🆕 | `ups.serial` | `9B2546A22161` | UPS序列号 |
| 🔵 | `ups.status` | `OL CHRG` | UPS状态 (OL/OB/LB/HB/RB/CHRG/DISCHRG/BYPASS/CAL/OFF/OVER/TRIM/BOOST/FSD) |
| 🔵 | `ups.test.result` | `Done and passed` | 自检结果 |
...
| 🔵 | `battery.runtime` | `3618` | 剩余运行时间(秒) |
| 🆕 | `battery.runtime.low` | `180` | 低运行时间阈值(秒) |
| 🔵 | `battery.type` | `PbAc` | 电池类型 |
| 🔵 | `battery.voltage` | `13.5` | 电池电压(V) |
...
| 🆕 | `device.serial` | `9B2546A22161` | 设备序列号 |
...
| 状态 | 变量名 | 值 | 描述 |
|:----:|--------|-----|------|
| 🆕 | `input.sensitivity` | `medium` | 输入灵敏度 |
| 🔵 | `input.transfer.high` | `266` | 高压转换阈值(V) |
| 🔵 | `input.transfer.low` | `180` | 低压转换阈值(V) |
| 🔵 | `input.voltage` | `218.0` | 输入电压(V) |
...
| `driver.parameter.subdriver` | apc | ⚙️ entrypoint.sh | APC 专用子驱动配置 |
| `driver.parameter.vendorid` | 051D | ⚙️ entrypoint.sh | 从 nut-scanner 提取后写入配置 |

### 其他驱动参数

| 变量名 | 当前值 | 说明 |
|--------|--------|------|
| `driver.parameter.productid` | 0002 | USB 产品 ID |
| `driver.parameter.port` | auto | 驱动端口 |

...
> 测试时间: 2026-03-04 22:26:48  
> NUT 驱动: nutdrv_qx 2.8.3
...
| 🆕 | `ups.delay.shutdown` | `30` | 关机延迟(秒) |
| 🆕 | `ups.delay.start` | `180` | 启动延迟(秒) |
| 🆕 | `ups.firmware.aux` | `PM-V` | UPS辅助固件版本 |
| 🔵 | `ups.load` | `0` | UPS负载(%) |
| 🆕 | `ups.productid` | `5161` | USB产品ID |
| 🔵 | `ups.status` | `OL` | UPS状态 (OL/OB/LB/HB/RB/CHRG/DISCHRG/BYPASS/CAL/OFF/OVER/TRIM/BOOST/FSD) |
| 🆕 | `ups.type` | `offline / line interactive` | UPS类型 (offline/line-int/online) |
| 🆕 | `ups.vendorid` | `0665` | USB厂商ID |
...
| 🔵 | `battery.voltage` | `13.6` | 电池电压(V) |
| 🆕 | `battery.voltage.high` | `13.00` | 电池高电压(V) |
| 🆕 | `battery.voltage.low` | `10.40` | 电池低电压(V) |
...
| 状态 | 变量名 | 值 | 描述 |
|:----:|--------|-----|------|
| 🔵 | `input.voltage` | `226.5` | 输入电压(V) |
| 🆕 | `input.voltage.fault` | `226.5` | 故障输入电压(V) |

### 📁 输出电源 (5 个变量)

| 状态 | 变量名 | 值 | 描述 |
|:----:|--------|-----|------|
| 🔵 | `output.current.nominal` | `4.0` | 额定输出电流(A) |
| 🔵 | `output.frequency` | `50.0` | 输出频率(Hz) |
| 🆕 | `output.frequency.nominal` | `50` | 额定输出频率(Hz) |
| 🔵 | `output.voltage` | `226.5` | 输出电压(V) |
| 🆕 | `output.voltage.nominal` | `220` | 额定输出电压(V) |
...
| 🆕 | `driver.parameter.productid` | `5161` | 产品ID参数 |
| 🆕 | `driver.parameter.synchronous` | `auto` | 同步模式 |
| 🆕 | `driver.parameter.vendorid` | `0665` | 厂商ID参数 |
...
| 状态 | 变量名 | 值 | 描述 |
|:----:|--------|-----|------|
| 🆕 | `driver.version.data` | `Voltronic-QS 0.09` | 数据版本 |
| 🆕 | `driver.version.internal` | `0.42` | 内部驱动版本 |
...
| `ups.productid` | `5161` | USB产品ID |
| `ups.status` | `OL` | UPS状态 (OL/OB/LB/HB/RB/CHRG/DISCHRG/BYPASS/CAL/OFF/OVER/TRIM/BOOST/FSD) |
| `ups.type` | `offline / line interac...` | UPS类型 (offline/line-int/online) |
| `ups.vendorid` | `0665` | USB厂商ID |
...
| `driver.parameter.vendorid` | 0665 | ⚙️ entrypoint.sh | 从 nut-scanner 提取后写入配置 |

### 其他驱动参数

| 变量名 | 当前值 | 说明 |
|--------|--------|------|
| `driver.parameter.productid` | 5161 | USB 产品 ID |
| `driver.parameter.port` | auto | 驱动端口 |
